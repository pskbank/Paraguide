...
//                rssi값을 60을 기준으로 처리, 스마트폰만 디스플레이 2013-12-24
// compile option:  gcc -o parapageap parapageap.c -lbluetooth -lpthread
**************************************************************************/
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
...
int check_network()
{
    char hname[30] = "www.parapage.com";
    struct hostent *hptr;
...
void update_dnode(char *addr)
{
    ...
    while(s != tail){  
        ...
        //if ( tt_min>=5)
        if ( tt_min>=1)  // 2013-12-17  1 분으로 수정 ( 테스트를 위한 임시 방편 )
        {
            delete_dnode(s->addr);
...
void create_daemon()
{
    ...
    if(pid > 0) {
      notify(stderr,"ParaguideAP Child process created\n");
      exit(0);
    }
...
void * thread_request_access( dnode *s)
{
    ...
    if ((hptr = gethostbyname(hname)) == NULL) {
        fprintf(stderr, " gethostbyname error for host: %s: %s\n",
            hname, hstrerror(h_errno));
        access_cnt++;  // 2013-07-22  updated by psk 
        return;
    }
    access_cnt=0;      // 2013-07-22  updated by psk
...
static void process_result(bdaddr_t *bdaddr, char has_rssi, int rssi,uint8_t *pcls)
{
       ...
       if(!(device_type1 == 2 && device_type2==3)) return;
       //if(rssi < -60) return;  // rssi 값이 -60보다 작으면 pass
...
static int inquiry_with_rssi(int sock)
{
       ...
       while(running) {
        ...
        //printf("ptime = %s\n",tbuf);
        // 오전 06:00에 자동으로 rebooting하도록 설정 or network failed 
        if(access_cnt > 5 || strcmp(tbuf,"0600")==0) system("shutdown -r now");
        sleep(0.5);        
...
int main(int argc, char **argv)
{
    ...
    switch(ch)
    {
       ...
       case  'r':
                   ...
                   reset_request();
       default:
                   printf("For daemon parapageap d , For test parapageap t\n");
                   return 0;
    }
