...
// compile option:  gcc -o officeap officeap.c -lbluetooth -lpthread
**************************************************************************/
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
...
void * thread_request_access( dnode *s)
{
    ...
    char **pptr;
    //********** Soomcloud setting *******
    char hname[30] = "office.soomcloud.com";
    char page[30] = "/guests/access";
    char poststr[80] = "guest[mac_address]=";
    if(s==NULL){
