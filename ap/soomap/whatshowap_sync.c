		    	...
		    	//		strcpy(name,"[unknown]");
                    ...
                    if(s != tail) // Mac address table에 이미 등록된 Mac address이면
                    {
                        ...
                        if(s->send_flag==1 && tt_min==3 ) {
                              ...
                              sprintf(print_buf,"[SEND_STAY_REQUEST===================> mac : %s name : %s ret_msg: %s\n",addr,name,tbuf);
                        ...
                        if(g_process_mode>2 || (s->send_flag==0 && s->send_cnt<5) ){
                              ...
                              sprintf(print_buf,"[RE request_notification]===========> mac : %s name : %s ret_msg: %s\n",addr,name,tbuf);
                              notify(stdout,print_buf); 
                        }

                        else if(s->send_flag!=0 || (s->send_flag==0 && s->send_cnt >= 5))
                        {
                             sprintf(print_buf,"[ Passed Bluetooth Device ] => mac : %s name : %s flag=%d cnt=%d\n",addr,name,s->send_flag,s->send_cnt);
                             notify(stdout,print_buf);
                        }
                    }
                    else   // Mac address table에 없다면  처음으로 서버에 노티를 요청
                    {
                          ...
                          if(s->send_flag==1) strcpy(tbuf,"Send Success");
  		                  ...
  		                  sprintf(print_buf,"[First request_notification]============> mac : %s name : %s ret_msg: %s\n",addr,name,tbuf);
                          notify(stdout,print_buf);
                    }
